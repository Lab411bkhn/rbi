...
from ProcessPython import MYSQL_CAL as DAL_CAL;

class DM_CAL:
    # ham khoi tao
    def __init__(self, APIComponentType= "",
                 Diametter = 0,NomalThick = 0,CurrentThick = 0,MinThickReq = 0,CorrosionRate = 0,CA = 0,ProtectedBarrier = False, CladdingCorrosionRate=0,InternalCladding = False,NoINSP_THINNING = 0,EFF_THIN = "E",OnlineMonitoring = "",HighlyEffectDeadleg = False,ContainsDeadlegs = False,TankMaintain653 = False,AdjustmentSettle = "", ComponentIsWeld = False,
                 LinningType = "", LINNER_ONLINE = False, LINNER_CONDITION = "", YEAR_IN_SERVICE = 0,INTERNAL_LINNING = False ,
                 CAUSTIC_INSP_EFF ="E", CAUSTIC_INSP_NUM = 0, HEAT_TREATMENT ="", NaOHConcentration = 0,HEAT_TRACE = False, STEAM_OUT= False,
                 AMINE_INSP_EFF = "E", AMINE_INSP_NUM = 0, AMINE_EXPOSED = False, AMINE_SOLUTION = "",
                 ENVIRONMENT_H2S_CONTENT = False, AQUEOUS_OPERATOR = False, AQUEOUS_SHUTDOWN =False,SULPHIDE_INSP_EFF = "E", SULPHIDE_INSP_NUM = 0,H2SContent = 0, PH = 0,PRESENT_CYANIDE = False,BRINNEL_HARDNESS = "",
                 SULFUR_INSP_EFF = "E", SULFUR_INSP_NUM = 0, SULFUR_CONTENT = "",
                 CACBONATE_INSP_EFF = "E", CACBONATE_INSP_NUM = 0, CO3_CONTENT = 0,
                 PTA_SUSCEP = False, NICKEL_ALLOY = False, EXPOSED_SULFUR = False, PTA_INSP_EFF = "E", PTA_INSP_NUM = 0, ExposedSH2OOperation = False, ExposedSH2OShutdown = False, ThermalHistory = "", PTAMaterial = "",DOWNTIME_PROTECTED = False,
                 INTERNAL_EXPOSED_FLUID_MIST = False, EXTERNAL_EXPOSED_FLUID_MIST = False, CHLORIDE_ION_CONTENT = 0, CLSCC_INSP_EFF = "E", CLSCC_INSP_NUM = 0,
                 ...
                 INTERFACE_SOIL_WATER = False, SUPPORT_COATING = False, INSULATION_TYPE = "", CUI_INSP_NUM = 0, CUI_INSP_EFF = "E", CUI_INSP_DATE = datetime.now().date(), CUI_PERCENT_1 = 0, CUI_PERCENT_2 = 0, CUI_PERCENT_3 = 0, CUI_PERCENT_4 = 0, CUI_PERCENT_5=0, CUI_PERCENT_6 = 0, CUI_PERCENT_7 = 0, CUI_PERCENT_8 = 0, CUI_PERCENT_9 = 0, CUI_PERCENT_10 = 0,
                 EXTERN_CLSCC_INSP_NUM = 0, EXTERN_CLSCC_INSP_EFF = "E",
                 EXTERNAL_INSULATION = False, COMPONENT_INSTALL_DATE = datetime.now().date(), CRACK_PRESENT = False, EXTERNAL_EVIRONMENT = "", EXTERN_COAT_QUALITY = "", EXTERN_CLSCC_CUI_INSP_NUM = 0, EXTERN_CLSCC_CUI_INSP_EFF = "E", PIPING_COMPLEXITY = "", INSULATION_CONDITION = "",INSULATION_CHLORIDE = False,
                 MATERIAL_SUSCEP_HTHA = False, HTHA_MATERIAL = "", HTHA_NUM_INSP = 0, HTHA_EFFECT = "E", HTHA_PRESSURE = 0, CRITICAL_TEMP = 0, DAMAGE_FOUND = False,
                 LOWEST_TEMP = False,
                 ...
                 EquipmentType = "", PREVIOUS_FAIL = "", AMOUNT_SHAKING = "", TIME_SHAKING = "", CYLIC_LOAD = "" , CORRECT_ACTION = "", NUM_PIPE = "",PIPE_CONDITION = "", JOINT_TYPE = "", BRANCH_DIAMETER = ""):
    ...
    def Art(self,age):
        if self.InternalCladding:
            return max(1 - (self.CurrentThick - self.CladdingCorrosionRate*self.agerc() - self.CorrosionRate*(age - self.agerc()))/ (self.getTmin() + self.CA),0);
        else:
            return max(1 - (self.CurrentThick - self.CorrosionRate*age)/(self.getTmin() + self.CA), 0 );

    def API_ART(self, a):
        art = 0;
        if self.APIComponentType != "TANKBOTTOM":
            data = [0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65];
            ...
            data = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1];
            if (a < (data[0] + data[1]) / 2):
                art = data[0];
            elif (a < (data[1] + data[2]) / 2):
                art = data[1];
    ...
    def DF_THIN(self, age):
        Fip = 1; Fdl = 1; Fwd = 1; Fam = 1; Fsm = 1; Fom = 1;
        ...
        if (self.OnlineMonitoring == "Amine high velocity corrosion - Electrical resistance probes" or self.OnlineMonitoring == "Amine high velocity corrosion - Key process variable" or self.OnlineMonitoring == "Amine low velocity corrosion - Electrical resistance probes" or self.OnlineMonitoring == "HCI corrosion - Electrical resistance probes" or
            ...
            Fom = 2;
        elif(self.OnlineMonitoring == "Amine low velocity corrosion - Key process variable" or self.OnlineMonitoring == "HCI corrosion - Key process variable & Electrical resistance probes" or self.OnlineMonitoring == "Sour water low velocity corrosion - Key process variable" or self.OnlineMonitoring == "Sulfuric acid (H2S/H2) corrosion high velocity - Key process parameters & electrical resistance probes" or self.OnlineMonitoring == "Sulfuric acid(H2S / H2) corrosion low velocity - Key process parameters"):
            Fom = 20;
        else:
            Fom = 1;
    ...
    def GET_SUSCEPTIBILITY_PTA(self):
        ...
        if(not self.ExposedSH2OOperation and not self.ExposedSH2OShutdown):
            ...
            if (self.MAX_OP_TEMP < 427):
                if (self.ThermalHistory == "Solution Annealed"):
                    if(self.PTAMaterial == "Regular 300 series Stainless Steels and Alloys 600 and 800" or self.PTAMaterial == "321 Stainless Steel"):
                        sus = "Medium";
                    elif(self.PTAMaterial == "H Grade 300 series Stainless Steels"):
                        sus = "High";
                    elif(self.PTAMaterial == "L Grade 300 series Stainless Steels" or self.PTAMaterial == "347 Stainless Steel, Alloy 20, Alloy 625, All austenitic weld overlay"):
                        sus = "Low";
                    else:
                        sus = "None";
                elif(self.ThermalHistory == "Stabilised Before Welding"):
                    ...
                    if(self.PTAMaterial == "321 Stainless Steel" or self.PTAMaterial == "347 Stainless Steel, Alloy 20, Alloy 625, All austenitic weld overlay"):
                        sus = "Low";
                    else:
                        sus = "None";
                else:
                ...
                if (self.ThermalHistory == "Solution Annealed"):
                    if(self.PTAMaterial == "Regular 300 series Stainless Steels and Alloys 600 and 800" or self.PTAMaterial == "H Grade 300 series Stainless Steels" or self.PTAMaterial == "321 Stainless Steel"):
                        sus = "High";
                    elif(self.PTAMaterial == "L Grade 300 series Stainless Steels" or self.PTAMaterial == "347 Stainless Steel, Alloy 20, Alloy 625, All austenitic weld overlay"):
                        sus = "Medium";
                    else:
    ...
    def API_EXTERNAL_CORROSION_TEMP(self):
        ...
        list = [self.CUI_PERCENT_1, self.CUI_PERCENT_2,self.CUI_PERCENT_3, self.CUI_PERCENT_4, self.CUI_PERCENT_5,self.CUI_PERCENT_6,self.CUI_PERCENT_7,self.CUI_PERCENT_8,self.CUI_PERCENT_9,self.CUI_PERCENT_10];
    ...
    def DF_EXTERNAL_CORROSION(self,age):
        if(self.EXTERNAL_EXPOSED_FLUID_MIST or (self.CARBON_ALLOY and not(self.MAX_OP_TEMP < -23 or self.MIN_OP_TEMP > 121))):
            ...
            if(self.APIComponentType == "TANKBOTTOM"):
                ...
                if(self.NomalThick == 0 or self.CurrentThick == 0):
                    ...
                    return DAL_CAL.MySQL_CAL.GET_TBL_511(self.API_ART_EXTERNAL(age), self.EXTERNAL_INSP_NUM, self.EXTERNAL_INSP_EFF);
        else:
            return 0;
    ...
    def API_ART_CUI(self, age):
        FIN = 1; FCM = 1; FIC = 1; FPS = 1; FIP = 1;
        if (self.INSULATION_TYPE == "Asbestos" or self.INSULATION_TYPE == "Calcium Silicate" or self.INSULATION_TYPE == "Mineral Wool" or self.INSULATION_TYPE == "Fibreglass"):
            FIN = 1.25;
    ...
    def DF_EXTERN_CLSCC(self):
        if(self.AUSTENITIC_STEEL and self.EXTERNAL_EXPOSED_FLUID_MIST and not(self.MAX_OP_TEMP < 49 or self.MIN_DESIGN_TEMP > 149)):
    ...
    def DF_CUI_CLSCC(self):
        if(self.AUSTENITIC_STEEL and self.EXTERNAL_INSULATION and self.EXTERNAL_EXPOSED_FLUID_MIST and not (self.MIN_OP_TEMP > 150 or self.MAX_OP_TEMP < 50)):
    ...
    def DFB_BRIITLE(self):
        ...
        if(self.PWHT):
            return DAL_CAL.MySQL_CAL.GET_TBL_215(self.API_TEMP(TEMP_BRITTLE - self.REF_TEMP), self.API_SIZE_BRITTLE(self.BRITTLE_THICK));
        else:
            return DAL_CAL.MySQL_CAL.GET_TBL_214(self.API_TEMP(TEMP_BRITTLE - self.REF_TEMP), self.API_SIZE_BRITTLE(self.BRITTLE_THICK));
    ...
    def DF_TEMP_EMBRITTLE(self):
        if(self.TEMPER_SUSCEP or (self.CARBON_ALLOY and not(self.MAX_OP_TEMP < 343 or self.MIN_OP_TEMP > 577))):
            ...
            if(self.PWHT):
                return DAL_CAL.MySQL_CAL.GET_TBL_215(self.API_TEMP(TEMP_EMBRITTLE), self.API_SIZE_BRITTLE(self.BRITTLE_THICK));
            else:
                return DAL_CAL.MySQL_CAL.GET_TBL_214(self.API_TEMP(TEMP_EMBRITTLE), self.API_SIZE_BRITTLE(self.BRITTLE_THICK));
        else:
            return 0;
    ...
    # Calculate Pipping
    def DFB_PIPE(self):
        DFB_PF = 1; DFB_AS = 1; FFB_AS = 1; DFB_CF = 1;
    ...
    def DF_PIPE(self):
        if(self.EquipmentType == "Piping"):
            ...
            if (self.PIPE_CONDITION == "Broken gussets or gussets welded directly to pipe" or self.PIPE_CONDITION == "Missing or damage supports, improper support"):
                FCP = 2;
            else:
                FCP = 1;
